...
class BusSend(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for n in ("alice", "bob", "carol"):
            tabus.bus_tac_add(cls.con, "t", n, "alice")
    ...
    def test_other_refusals_carry_no_code(self):
        ...
        self.assertEqual(info, "no such tac: no-such-tac")
        self.assertFalse(hasattr(info, "code"))
    ...
    def test_unread_blocked_dm_and_tac(self):
        ...
        tabus.bus_send(self.con, "carol", [], "t1", "one", tac_id="t")
        tabus.bus_send(self.con, "carol", [], "t2", "two", tac_id="t")
        mid, info = tabus.bus_send(self.con, "alice", [], "r", "reply", tac_id="t")
        self.assertTrue(info.startswith("tac 't': 2 unread. Read them first"))
        self.assertEqual(info.details, {"scope": "tac", "tac": "t", "unread": 2})
