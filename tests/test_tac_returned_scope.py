...
class ReturnedScopeTest(unittest.TestCase):
    def setUp(self):
        ...
        for topic in ("first", "second"):
            ok, message = tabus.bus_tac_create(self.con, topic)
            self.assertTrue(ok, message)
            self.tacs[topic] = self.con.execute(
                "SELECT tac_id FROM tacs WHERE name=?", (topic,)
            ).fetchone()[0]
            for node in ("sender", "reader", "other"):
                self.assertTrue(
                    tabus.bus_tac_add(self.con, self.tacs[topic], node)[0]
                )
        self.ids = [
            tabus.bus_send(
                self.con, "sender", [], str(i), "body", tac_id=self.tacs["first"]
            )[0]
            for i in range(3)
        ]
        self.foreign = tabus.bus_send(
            self.con, "sender", [], "foreign", "body", tac_id=self.tacs["second"]
        )[0]
    ...
    def test_limit_selection_and_repeat(self):
        ...
        sent, error = tabus.bus_send(
            self.con, "reader", [], "reply", "body", tac_id=first
        )
    ...
    def test_empty_and_cross_topic_selection(self):
        ...
        self.assertEqual(
            0, tabus.bus_tac_mark_read(self.con, "reader", first, [self.foreign])
        )
