...
Isolation: run busd in a separate process with a temporary database, not a shared client file.
"""
...
def main():
    ...
    # 🔴 The modules use package-relative imports, so they run as `-m tabus.x`, not as a
    #    file path. Subprocesses find the package through PYTHONPATH (sys.path does not
    #    cross a spawn).
